...
def add_product_highlight(df, product_col, value_col):
...
excel_file = r"C:\Users\Josué Obed\OneDrive - Instituto Tecnologico y de Estudios Superiores de Monterrey\TEC\6TH SEMESTER\MODWEB\PANEM FINAL\Panem_Dashboard_Datasource.xlsx"

if not os.path.exists(excel_file):
    st.error("Excel file not found. Put Panem_Dashboard_Datasource.xlsx in the same folder as app.py.")
    st.stop()
...
if os.path.exists("panem_logo.png"):
    st.sidebar.image("panem_logo.png", use_container_width=True)
else:
...
if page == "1. Branch Forecast":
    ...
    if "recent_7_day_baseline" in product_summary.columns:
        product_summary["difference_vs_recent_7_day"] = (
            product_summary["predicted"] - product_summary["recent_7_day_baseline"]
        )
    ...
    kpi1.metric("Predicted Units", f"{total_predicted_units:,.0f}")
    kpi2.metric("Top Product", top_product)
    kpi3.metric("Estimated Revenue", f"${total_revenue:,.2f}")
    kpi4.metric("Prediction Confidence", confidence)

    st.subheader("Product Forecast")

    product_summary = product_summary.sort_values("predicted", ascending=False)
    product_summary = add_product_highlight(product_summary, "product", "predicted")
    ...
    fig_products = px.bar(
        ...
        title=f"Expected units by product for {selected_branch}",
        text_auto=True
    )
    ...
    if not monthly_context.empty:
        ...
        fig_month.update_yaxes(categoryorder="total ascending")
        ...
        st.info("No historical monthly context available for this branch and forecast month.")
...
# =====================================================
    ...
    top_products_chart = add_product_highlight(
        top_products_chart,
        "item",
        "quantity"
    )
...
# =====================================================
    ...
    error_comparison = error_comparison.rename(
        columns={
            ...
            "Recent_7_Day_MAPE": "Recent 7-day baseline"
        }
    )
    ...
    method_colors = {
        "Model": highlight_color,
        "Historical baseline": base_color,
        "Recent 7-day baseline": "#6f8f72"
    }
    ...
    fig_error_compare = px.bar(
        ...
        title="Lower error means more reliable predictions",
        text_auto=True
    )
...
# =====================================================
    ...
    if uploaded_file is not None:
        ...
        missing_update_cols = [
            col for col in required_update_cols
